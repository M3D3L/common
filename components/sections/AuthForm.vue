...
const redirectPath = computed(() => {
  ...
  // Ensure the source is a safe relative path starting with '/'
  // Defaults to '/' if the source param is missing or invalid.
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          ...
          const loginPath =
            redirectPath.value === "/"
              ? "/login"
              : `/login?source=${encodeURIComponent(redirectPath.value)}`;
