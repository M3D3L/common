<template>
  <div id="top" class="w-full scroll-mt-40">
    ...
    >
      <template
        v-for="(section, sectionIn) in sections"
        :key="`pill-${section.id}`"
      >
        <Button
...
const activeSection = ref(
  props.initialActiveId ?? props.sections?.[0]?.id ?? null
);
const sectionRefs = ref([]);
let observer = null;
let isLocked = false;

// Arrow function for debounce
...
// Arrow function for handlePillClick
const handlePillClick = (id, event) => {
  emit("pillClick", id);
  if (!process.client) return;
...
// Arrow function for setSectionRef
