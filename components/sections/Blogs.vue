<template>
    <containers-video id="blogs" video="https://videos.pexels.com/video-files/5561385/5561385-uhd_2560_1440_25fps.mp4"
        :title="title" :description="description" class="min-h-[90vh] md:min-h-screen">
        <template #video-container-content>
                        ...
                        <CardContent class="flex flex-col items-center gap-6 p-0 md:flex-row">
                            <div class="order-2 w-full px-6 py-6 md:order-1 md:w-1/2 md:px-8 md:py-12">
                                ...
                                <div class="flex items-center gap-3 mt-6">
                                    ...
                                    <span class="text-sm text-muted-foreground" v-if="featuredPost.description">
                                        {{ calculateReadingTime(featuredPost.description) }} min read
                                    </span>
                                </div>
                            </div>
                            <div
                                class="order-1 w-full overflow-hidden md:order-2 md:w-1/2 max-h-[450px] rounded-lg aspect-square lg:aspect-video shadow-md">
                                <NuxtLink :to="getBlogUrl(featuredPost?.created, featuredPost?.slug)">
                                    <img :src="featuredPost.cover_image || placeholderImage"
            ...
            <motion.div :variants="fadeUp" class="grid grid-cols-1 gap-6 mt-12 mb-8 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <CardHeader class="p-0 border-b border-muted">
                            <div class="relative overflow-hidden aspect-video">
                                <NuxtLink :to="getPostLink(post)">
                                    <img :src="post.cover_image || placeholderImage" :alt="post.title" width="600"
                ...
                <Card class="relative h-full overflow-hidden group">
                    ...
                    <CardContent class="flex flex-col justify-end h-full p-6">
                        <NuxtLink to="/blog"
                            class="inline-flex items-center self-start px-4 py-2 mt-4 text-sm font-medium text-white transition rounded-lg bg-primary hover:bg-primary-600">
                            View All Posts
