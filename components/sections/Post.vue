<template>
  <article
     class="min-h-[90vh] md:min-h-screen relative z-10 flex items-center w-full h-full pb-24">
     <motion.div class="w-full max-w-4xl mx-auto" initial="hidden" whileInView="visible"
        ...
        <TooltipProvider>
                       ...
                       <Tooltip>
                          <TooltipTrigger as-child>
                             <Button variant="outline" size="sm" @click="scrollToComments"
                                ...
                                Comments
                                <span class="ml-1">
                                ({{ post.expand['comments(post)']?.length || 0 }})
                                </span>
                             </Button>
                          </TooltipTrigger>
                          <TooltipContent>
           ...
           <SubmitComment v-if="post?.id" 
              @comment-created="refresh()" 
              :post="post.expand?.['comments(post)'] || []" 
              :id="post.id" />
