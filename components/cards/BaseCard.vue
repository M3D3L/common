    ...
    <CardHeader class="p-0 border-b border-muted">
      ...
      >
        <div class="relative w-full h-auto aspect-video">
          <img
            :src="image || createImgUrl(content)"
            :alt="content.title"
    ...
    <CardContent :class="!removeSpacing ? 'lg:p-8' : 'lg:p-6'" class="p-6">
      <div class="flex flex-col h-full">
        <div :class="{ 'h-40': !removeSpacing }">
          ...
          <p
            v-if="content?.sub_title"
            class="mt-2 text-sm text-muted-foreground"
          >
            {{ content.sub_title }}
          </p>
          <p :class="{ 'h-20': !removeSpacing }" class="mt-3 text-sm">
            {{ content.description }}
          </p>
        </div>
        ...
        <div v-if="content?.tech" class="flex flex-wrap gap-2 mt-6">
          <Badge
            v-for="tech in content.tech.slice(0, 3)"
            :key="tech"
            variant="outline"
            class="text-xs"
          >
