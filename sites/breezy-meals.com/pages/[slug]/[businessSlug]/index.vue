    ...
    <template v-else-if="business">
      ...
      >
        <TitleBlock
          ...
          :title="business.name"
          :description="
            isSpanish ? business.description_Sp : business.description_En
          "
        />
      ...
      <button
        ...
        class="fixed flex flex-col items-center content-center gap-2 right-4 bottom-4 z-50 bg-white dark:bg-gray-800 p-3 rounded-lg shadow-lg"
      >
        <img
          :src="
            isSpanish
      ...
      >
        <ModalCarousel
          v-if="business?.promoGallery?.length"
      ...
      <div class="container mx-auto font-body lg:flex lg:gap-8 lg:pt-16">
        ...
        <main class="space-y-12 lg:w-2/3">
          ...
          >
            ...
            <ModalCarousel
              v-if="business?.gallery?.length"
              :slides="business.gallery"
              :collection-id="business?.collectionId"
              :propertyId="business?.id"
              @selected-event="openModalWithImage"
              :aspectRatio="'aspect-video'"
          ...
          <section id="menu" class="scroll-mt-20" ref="setSectionRef">
            ...
            >
              <!-- Text content -->
              <div
                class="z-10 flex flex-col items-center space-y-3 text-center md:items-start md:text-left"
              >
                ...
                <NuxtLink
                  :to="`${route.path.endsWith('/') ? route.path : `${route.path}/`}menu`"
                >
                  <Button class="mt-2">
              ...
              <div
                class="relative flex items-center justify-center w-full md:w-auto opacity-20 md:opacity-100"
              >
              ...
              <div
                class="absolute inset-0 pointer-events-none bg-gradient-to-tr from-primary/5 to-transparent"
              />
            </Card>
          </section>
        </main>
...
const loadBusinessData = async () => {
  ...
  const businessSlug = route.params.businessSlug as string;
  if (!slug) {
    error.value = new Error(
      isSpanish.value ? "Falta el slug." : "Missing slug.",
    );
    loading.value = false;
    return;
  }
  ...
  try {
    // Fetch single business by slug filter
    const result = await fetchCollection(
      slug, // collection name
      1, // page
      1, // perPage - only get 1 item
      `slug="${businessSlug}"`, // filter by slug and businessSlug
      "-created", // sort
      "socials,services", // expand relations
    );

    if (result.items && result.items.length > 0) {
      business.value = result.items[0];
    } else {
      throw new Error(
...
const businessDescription = computed(() =>
  isSpanish.value
    ? business.value?.description_Sp || business.value?.description_ES
...
const businessGallery = computed(
  () =>
    business.value?.gallery?.map((g: any) => ({
      url: g.url || (typeof g === "string" ? g : placeholderUrl("Image")),
...
const loadingBusinessText = computed(() =>
  isSpanish.value ? "Cargando datos..." : "Loading business data...",
);
...
const contactTitle = computed(() => (isSpanish.value ? "Contacto" : "Contact"));
const hoursTitle = computed(() => (isSpanish.value ? "Horario" : "Hours"));
...
const servicesTitle = computed(() =>
  isSpanish.value ? "Servicios" : "Services",
);
...
function formatDay(day: string) {
  ...
  return isSpanish.value ? daysEs[day] || day : day;
}
...
const seoData = computed(() => ({
  title: business.value?.name || "Loading...",
  summary: businessDescription.value || "Check out this local business!",
