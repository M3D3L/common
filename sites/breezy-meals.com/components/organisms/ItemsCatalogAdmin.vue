...
const isFlatListSurface = computed(() => props.dishes === "catering");
...
async function load() {
  ...
  try {
    ...
    if (isFlatListSurface.value) {
      menuRecordId.value = legacy.id;
      catalog.value = normalizeMenuCatalog(
        asSurfaceCatalog(legacy[props.dishes], props.dishes),
      );
      return;
    }
    ...
    catalog.value = normalizeMenuCatalog(
      asSurfaceCatalog(record[props.dishes], props.dishes),
    );
  } catch {
...
async function save() {
  ...
  try {
    ...
    if (menuRecordId.value) {
      if (isFlatListSurface.value) {
        await updateItem(props.fetchedCollection, menuRecordId.value, {
          [props.dishes]: menuItems[props.dishes] ?? [],
        });
      } else {
      ...
      const created = await createItem("menu", {
        [props.dishes]: isFlatListSurface.value
          ? (menuItems[props.dishes] ?? [])
          : menuItems,
